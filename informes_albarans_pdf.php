...
// Ajusta el path segons on tens TCPDF
...
if ($dataInici !== '') {
    $where[] = "a.data_albara >= :data_inici";
    $params['data_inici'] = $dataInici;
}
if ($dataFi !== '') {
    $where[] = "a.data_albara <= :data_fi";
...
// Consulta d'albarans amb total calculat
$sql = "
    SELECT a.id,
           a.num_albara,
           a.data_albara,
    ...
    $whereSql
    GROUP BY a.id, a.num_albara, a.data_albara, c.nom
    ORDER BY a.data_albara, a.num_albara
";
...
if ($action === 'pdf') {
    ...
    $html .= '<tr style="font-weight:bold;background-color:#eeeeee;">
                <td width="18%">Núm. albarà</td>
                <td width="16%">Data</td>
                <td width="46%">Client</td>
                <td width="20%" align="right">Import</td>
              </tr>';

    foreach ($albarans as $a) {
        $dataTxt = $a['data_albara']
            ? (new DateTime($a['data_albara']))->format('d/m/Y')
            : '';
        $html .= '<tr>
                    <td>'.htmlspecialchars($a['num_albara']).'</td>
                    <td>'.$dataTxt.'</td>
                    <td>'.htmlspecialchars($a['client_nom']).'</td>
                    <td align="right">'.number_format((float)$a['import_total'], 2, ',', '.').' €</td>
                  </tr>';
    }

    if (empty($albarans)) {
        $html .= '<tr><td colspan="4" align="center">No hi ha albarans per als filtres indicats.</td></tr>';
    }

    $html .= '</table>';

    $pdf->writeHTML($html, true, false, true, false, '');
    $pdf->Output('llistat_albarans.pdf', 'D');
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Llistat d'albarans (PDF)</title>
    <style>
        :root {
            --bg: #020617;
...
<div class="page">
    ...
    <div class="list-card">
        <?php if (empty($albarans)): ?>
            <div class="empty">No hi ha albarans per als filtres indicats.</div>
        <?php else: ?>
            <table>
                <thead>
                <tr>
